...
class PerformanceTracker:
    ...
    def get_performance_metrics(self, days: Optional[int] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_profit_per_winning_trade": round(total_profit / len(profitable), 2)
            if profitable
            else 0,
            "avg_loss_per_losing_trade": round(total_loss / len(losing), 2)
            if losing
            else 0,
                ...
                daily_stats.append(
                    {
                        ...
                        "losers": stats["losers"],
                        "win_rate": round((stats["winners"] / stats["trades"]) * 100, 2)
                        if stats["trades"] > 0
                        else 0,
                    }
                )
