...
class DynamicPositionSizer:
    ...
    def get_volatility_multiplier(self, symbol: str) -> Tuple[float, str]:
        ...
        if volatility < self.VOL_LOW_THRESHOLD:
            return self.VOL_LOW_MULTIPLIER, f"Low volatility ({vol_pct:.1f}% daily) -> 1.2x size"
        elif volatility > self.VOL_HIGH_THRESHOLD:
            return self.VOL_HIGH_MULTIPLIER, f"High volatility ({vol_pct:.1f}% daily) -> 0.75x size"
        else:
            # Linear interpolation between thresholds
            vol_range = self.VOL_HIGH_THRESHOLD - self.VOL_LOW_THRESHOLD
            vol_position = (volatility - self.VOL_LOW_THRESHOLD) / vol_range
            multiplier = self.VOL_LOW_MULTIPLIER - vol_position * (self.VOL_LOW_MULTIPLIER - self.VOL_HIGH_MULTIPLIER)
            return multiplier, f"Medium volatility ({vol_pct:.1f}% daily) -> {multiplier:.2f}x size"
    ...
    def should_deploy_cash(self, portfolio_info: Dict) -> Tuple[bool, str]:
        ...
        if cash_pct > self.CASH_DEPLOYMENT_TRIGGER_PCT:
            excess_cash = portfolio_info["cash"] - (portfolio_info["portfolio_value"] * self.MIN_CASH_RESERVE_PCT)
            return True, f"Cash at {cash_pct*100:.1f}% (>${excess_cash:,.0f} deployable)"
    ...
    def get_max_positions_with_cash_deployment(
        ...
        should_deploy, deploy_reason = self.should_deploy_cash(portfolio_info)
        ...
        extra_positions = min(extra_positions, qualified_count)  # Don't exceed qualified candidates
    ...
    def calculate_position_size(
        ...
        rationale.append(f"Base size: ${base_size:,.0f} (portfolio/${target_positions} positions)")
        
        # 2. Apply conviction multiplier
        conviction_mult, conviction_reason = self.get_conviction_multiplier(composite_score)
        ...
        rationale.append(f"Raw size: ${base_size:,.0f} x {conviction_mult:.2f} x {vol_mult:.2f} = ${raw_size:,.0f}")
    ...
    def calculate_batch_sizes(
        ...
        results = []
        remaining_cash = portfolio_info["cash"] - (portfolio_info["portfolio_value"] * self.MIN_CASH_RESERVE_PCT)
        ...
        for candidate in sorted_candidates:
            ...
            if remaining_cash < portfolio_info["portfolio_value"] * self.MIN_POSITION_PCT:
                logger.info("batch_sizing_stopped", reason="insufficient_remaining_cash")
                break
    ...
    def record_entry(self, symbol: str):
        ...
        logger.info("entry_recorded", symbol=symbol, daily_count=self._daily_entries[today])
