...
class ParameterSweepBacktest:
        ...
        for date in trading_days:
            ...
            for symbol, price, reason in to_exit:
                ...
                self.portfolio.trades.append(Trade(
                    ...
                    pnl=pnl, pnl_pct=(price/pos["entry_price"]-1)*100, exit_reason=reason
                ))
            ...
            if len(self.portfolio.positions) < self.max_positions:
                ...
                for symbol in symbols:
                    ...
                    if score >= self.min_score:
                        candidates.append({"symbol": symbol, "score": score, "price": current_prices.get(symbol)})
                ...
                for c in candidates[:slots]:
                    if c["price"]:
                        pos_value = min(self.portfolio.cash * 0.95, self.portfolio.cash / (slots + 1))
                        ...
                        if shares > 0:
                            ...
                            if cost <= self.portfolio.cash:
                                ...
                                self.portfolio.positions[c["symbol"]] = {
                                    ...
                                    "take_profit": c["price"] * (1 + self.take_profit_pct),
                                }
    ...
    def _calc_metrics(self) -> Dict:
        ...
        return {
            ...
            "avg_winner": round(np.mean([t.pnl_pct for t in winners]), 2) if winners else 0,
            "avg_loser": round(np.mean([t.pnl_pct for t in losers]), 2) if losers else 0,
...
def run_sweep():
    ...
    symbols = [
        ...
        "XOM", "CVX", "COP", "SLB", "EOG", "NEE", "DUK", "SO", "D", "AEP", "SPY"
    ]
    ...
    print(f"Total combinations: {len(stop_losses) * len(take_profits) * len(min_scores)}")
    print("="*70)
    ...
    print("="*70)
    print(f"{'SL':>5} {'TP':>5} {'Score':>6} {'Return':>8} {'Alpha':>8} {'WinRate':>8} {'MaxDD':>8} {'PF':>6} {'Trades':>7}")
    print("-"*70)
    
    for r in results[:10]:
        sl_str = f"{r['stop_loss']*100:.0f}%"
        tp_str = f"{r['take_profit']*100:.0f}%"
        print(f"{sl_str:>5} {tp_str:>5} {r['min_score']:>6.0f} "
    ...
    print(f"Combinations that beat SPY: {len(profitable)}/{len(results)} ({len(profitable)/len(results)*100:.1f}%)")
