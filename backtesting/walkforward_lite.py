...
def run_backtest(data_cache, symbols, trading_days, stop_loss, take_profit, min_score, initial_capital=100000):
...
def run_test():
    ...
    print(f"Testing {len(param_sets)} parameter sets across {len(periods)} market periods")
    print("="*80)
    ...
    for period in periods:
        p_start = datetime.strptime(period["start"], "%Y-%m-%d").replace(tzinfo=timezone.utc)
        p_end = datetime.strptime(period["end"], "%Y-%m-%d").replace(tzinfo=timezone.utc)
        ...
        spy_ret = (spy_period.iloc[-1]["close"] / spy_period.iloc[0]["open"] - 1) * 100 if len(spy_period) > 1 else 0
        ...
        for params in param_sets:
            result = run_backtest(data_cache, symbols, trading_days, params["sl"], params["tp"], params["ms"])
            if result:
                ...
                print(f"  {params['name']}: {result['total_return']:+.1f}% (alpha: {result['total_return']-spy_ret:+.1f}%)")
    ...
    for params in param_sets:
        ...
        print("-"*70)
        print(f"{'Period':<25} {'Return':>10} {'Alpha':>10} {'MaxDD':>10} {'WinRate':>10}")
        print("-"*70)
        ...
        for r in results[params["name"]]:
            print(f"{r['period']:<25} {r['return']:>+9.1f}% {r['alpha']:>+9.1f}% {r['max_dd']:>9.1f}% {r['win_rate']:>9.1f}%")
    ...
    for params in param_sets:
        ...
        print(f"{params['name']}: Total Alpha = {total_alpha:+.1f}%, Positive periods = {positive_periods}/{len(periods)}")
