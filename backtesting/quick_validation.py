...
class QuickBacktest:
    ...
    def load_data(self, symbols: list[str]) -> None:
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            print(
                f"  Loading batch {i//batch_size + 1}/{(len(symbols)-1)//batch_size + 1}..."
            )
    ...
    def get_price_on_date(self, symbol: str, date: datetime) -> float | None:
    ...
    def run(self, symbols: list[str]) -> dict:
        ...
        trading_days = spy_data[
            (spy_data.index >= self.start_date) & (spy_data.index <= self.end_date)
        ].index.tolist()

        print(f"\nRunning backtest over {len(trading_days)} trading days...")
        print(
            f"Strategy: min_score={self.min_score}, stop={self.stop_loss_pct:.1%}, tp={self.take_profit_pct:.1%}"
        )

        for i, date in enumerate(trading_days):
            if i % 50 == 0:
                print(
                    f"  Day {i}/{len(trading_days)} - {date.strftime('%Y-%m-%d')} - Positions: {len(self.portfolio.positions)}"
                )
    ...
    def _enter_position(
        self, symbol: str, date: datetime, price: float, score: float
    ) -> None:
        """Enter a new position."""
        # Position sizing: equal weight
        position_value = self.portfolio.cash / (
            self.max_positions - len(self.portfolio.positions) + 1
        )
        position_value = min(
            position_value, self.portfolio.cash * 0.95
    ...
    def _exit_position(
        self, symbol: str, date: datetime, price: float, reason: str
    ) -> None:
        """Exit a position."""
    ...
    def _calculate_results(self, trading_days: list) -> dict:
        ...
        equity_curve["drawdown"] = (
            (equity_curve["equity"] - equity_curve["peak"]) / equity_curve["peak"] * 100
        )
        ...
        if len(equity_curve) > 1:
            ...
            sharpe = (
                equity_curve["daily_return"].mean()
                / equity_curve["daily_return"].std()
                * np.sqrt(252)
            )
        else:
            sharpe = 0
...
def run_validation():
    ...
    spy_period["dd"] = (
        (spy_period["close"] - spy_period["peak"]) / spy_period["peak"] * 100
    )
