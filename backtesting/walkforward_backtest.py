...
def run_walkforward():
    ...
    for w_idx, window in enumerate(windows):
        train_start = datetime.strptime(window["train_start"], "%Y-%m-%d").replace(
            tzinfo=UTC
        )
        train_end = datetime.strptime(window["train_end"], "%Y-%m-%d").replace(
            tzinfo=UTC
        )
        test_start = datetime.strptime(window["test_start"], "%Y-%m-%d").replace(
            tzinfo=UTC
        )
        test_end = datetime.strptime(window["test_end"], "%Y-%m-%d").replace(tzinfo=UTC)

        print(f"\n{'='*70}")
        print(
            f"Window {w_idx+1}: Train {window['train_start']} to {window['train_end']}"
        )
        ...
        print(
            f"  Best train params: SL={best_params['stop_loss']:.0%}, TP={best_params['take_profit']:.0%}, Score={best_params['min_score']}"
        )
        ...
        if test_result:
            ...
            spy_test = spy_data[
                (spy_data.index >= test_start) & (spy_data.index <= test_end)
            ]
            if len(spy_test) > 1:
                spy_test_return = (
                    spy_test.iloc[-1]["close"] / spy_test.iloc[0]["open"] - 1
                ) * 100
            else:
                spy_test_return = 0
            ...
            print(
                f"  TEST RESULT: {test_result['total_return']:+.1f}% (SPY: {spy_test_return:+.1f}%, Alpha: {oos_result['alpha']:+.1f}%)"
            )
    ...
    print("=" * 70)
    print(
        f"{'Window':>8} {'Period':>25} {'Strategy':>10} {'SPY':>10} {'Alpha':>10} {'MaxDD':>8} {'Trades':>7}"
    )
    print("-" * 70)
    ...
    for r in oos_results:
        print(
            f"{r['window']:>8} {r['test_period']:>25} {r['strategy_return']:>+9.1f}% {r['spy_return']:>+9.1f}% "
    ...
    print("-" * 70)
    print(
        f"{'TOTAL':>8} {'Cumulative (compounded)':>25} {cumulative_strategy:>+9.1f}% {cumulative_spy:>+9.1f}% {cumulative_alpha:>+9.1f}%"
    )
