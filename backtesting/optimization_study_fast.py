...
class TechnicalScorer:
    ...
    def calculate_score(self, df):
        ...
        rsi_score = (
            50.0
            if pd.isna(rsi)
            else (100.0 if rsi < 30 else (0.0 if rsi > 70 else 100 - abs(rsi - 50) * 2))
        )
        macd_score = (
            50.0 if pd.isna(macd_hist) else min(100, max(0, 50 + macd_hist * 25))
        )
...
def prepare_sgd_data(data_cache, symbols, trading_days, stop_loss, take_profit):
    ...
    for i, date in enumerate(sample_days[:-10]):
        for symbol in symbols[:15]:  # Subset of symbols
            ...
            future = df[
                (df.index > date)
                & (df.index <= sample_days[min(i + 10, len(sample_days) - 1)])
            ]
            if len(future) < 3:
                continue
...
def main():
    ...
    spy_ret = (
        spy[spy.index <= end].iloc[-1]["close"]
        / spy[spy.index >= start].iloc[0]["open"]
        - 1
    ) * 100
    ...
    for name, sl, tp, ms in grids:
        ...
        if r:
            ...
            print(
                f'  {name}: {r["return"]:+.1f}% (alpha: {alpha:+.1f}%), DD: {r["max_dd"]:.1f}%'
            )
            results[f"Grid_{name}"] = {
                "return": r["return"],
                "alpha": alpha,
                "max_dd": r["max_dd"],
                "win_rate": r["win_rate"],
    ...
    print(
        f'  SGD Weights: RSI={sgd_weights["rsi"]:.2f}, MACD={sgd_weights["macd"]:.2f}, BB={sgd_weights["bb"]:.2f}, Trend={sgd_weights["trend"]:.2f}'
    )
    print(f"  SGD Threshold: {sgd_thresh:.1f}")

    spy_test_ret = (
        spy[(spy.index >= test_days[0]) & (spy.index <= test_days[-1])].iloc[-1][
            "close"
        ]
        / spy[(spy.index >= test_days[0]) & (spy.index <= test_days[-1])].iloc[0][
            "open"
        ]
        - 1
    ) * 100
    ...
    sgd_r = run_backtest(
        data_cache, symbols, test_days, 0.03, 0.08, sgd_thresh, sgd_weights
    )
    ...
    if sgd_r:
        ...
        results["SGD"] = {
            ...
            "alpha": alpha,
            "max_dd": sgd_r["max_dd"],
            "win_rate": sgd_r["win_rate"],
            "method": "sgd",
            "weights": sgd_weights,
            "threshold": sgd_thresh,
        }

    if default_r:
        alpha = default_r["return"] - spy_test_ret
        print(f'  Default (test): {default_r["return"]:+.1f}% (alpha: {alpha:+.1f}%)')
        results["Default_test"] = {
            "return": default_r["return"],
            "alpha": alpha,
            "max_dd": default_r["max_dd"],
            "win_rate": default_r["win_rate"],
            "method": "grid",
        }
    ...
    if OPTUNA_AVAILABLE:
        ...
        train_spy_ret = (
            spy[(spy.index >= train_days[0]) & (spy.index <= train_days[-1])].iloc[-1][
                "close"
            ]
            / spy[(spy.index >= train_days[0]) & (spy.index <= train_days[-1])].iloc[0][
                "open"
            ]
            - 1
        ) * 100
        ...
        print(
            f'  Best: SL={bp["sl"]*100:.1f}%, TP={bp["tp"]*100:.1f}%, Score={bp["ms"]:.1f}'
        )

        bay_r = run_backtest(
            data_cache, symbols, test_days, bp["sl"], bp["tp"], bp["ms"]
        )
        if bay_r:
    ...
    if regime_r:
        ...
        results["Regime"] = {
            "return": regime_r["return"],
            "alpha": alpha,
            "max_dd": regime_r["max_dd"],
            "win_rate": regime_r["win_rate"],
            "method": "regime",
            "regimes": regime_r["regimes"],
        }
    ...
    for name, r in sorted(
        ...
        print(
            f'{name:<20} {r["return"]:>+9.1f}% {r["alpha"]:>+9.1f}% {r["max_dd"]:>9.1f}% {r["win_rate"]:>9.1f}%'
        )
    ...
    if "Regime" in results:
        regime_diff = results["Regime"]["alpha"] - results.get(
            "Grid_Tight Entry", {}
        ).get("alpha", 0)
        print(
