...
class SGDWeightOptimizer:
    ...
    def prepare_training_data(
        self, data_cache, symbols, trading_days, stop_loss, take_profit
    ):
        ...
        for i, date in enumerate(trading_days[:-20]):
            for symbol in symbols:
                ...
                future_mask = (df.index > date) & (
                    df.index <= trading_days[min(i + 20, len(trading_days) - 1)]
                )
    ...
    def train(self, X, y, epochs=100, lr=0.01):
            ...
            if epoch % 20 == 0:
                with torch.no_grad():
                    ...
                    self.training_history.append(
                        {"epoch": epoch, "loss": loss.item(), "accuracy": acc.item()}
                    )
        ...
        self.best_weights = {
            "rsi": float(weights[0]),
            "macd": float(weights[1]),
            "bb": float(weights[2]),
            "trend": float(weights[3]),
        }
...
class MLEntryModel:
    ...
    def extract_features(self, df):
        ...
        features = [
            rsi if not pd.isna(rsi) else 50,
            macd_hist if not pd.isna(macd_hist) else 0,
            bb_position if not pd.isna(bb_position) else 0.5,
            trend_strength if not pd.isna(trend_strength) else 0,
            volume_ratio if not pd.isna(volume_ratio) else 1,
            atr_pct if not pd.isna(atr_pct) else 2,
            momentum_5d if not pd.isna(momentum_5d) else 0,
            momentum_20d if not pd.isna(momentum_20d) else 0,
        ]
...
def run_regime_adaptive_backtest(
    data_cache, symbols, trading_days, spy_data, initial_capital=100000
):
    ...
    for date in trading_days:
        ...
        stop_loss, take_profit, min_score = (
            params["stop_loss"],
            params["take_profit"],
            params["min_score"],
        )
...
def main():
    ...
    for p in grid_params:
        result = run_backtest_with_weights(
            data_cache, symbols, trading_days, p["sl"], p["tp"], p["ms"]
        )
        if result:
            alpha = result["total_return"] - spy_return
            print(
                f"  {p['name']}: Return={result['total_return']:+.1f}%, Alpha={alpha:+.1f}%, MaxDD={result['max_drawdown']:.1f}%"
            )
            results[f"Grid: {p['name']}"] = {
    ...
    X_train, y_train = sgd_optimizer.prepare_training_data(
        data_cache, symbols, train_days, 0.03, 0.08
    )
    print(
        f"  Training samples: {len(X_train)}, Positive rate: {y_train.mean()*100:.1f}%"
    )

    print("  Training SGD model...")
    best_weights, optimal_threshold = sgd_optimizer.train(
        X_train, y_train, epochs=200, lr=0.05
    )

    print(
        f'  Optimized Weights: RSI={best_weights["rsi"]:.3f}, MACD={best_weights["macd"]:.3f}, BB={best_weights["bb"]:.3f}, Trend={best_weights["trend"]:.3f}'
    )
    print(f"  Optimal Threshold: {optimal_threshold:.1f}")

    spy_test = spy_data[
        (spy_data.index >= test_days[0]) & (spy_data.index <= test_days[-1])
    ]
    spy_test_return = (spy_test.iloc[-1]["close"] / spy_test.iloc[0]["open"] - 1) * 100

    sgd_result = run_backtest_with_weights(
        data_cache, symbols, test_days, 0.03, 0.08, optimal_threshold, best_weights
    )
    if sgd_result:
        sgd_alpha = sgd_result["total_return"] - spy_test_return
        print(
            f'  SGD Result (Test): Return={sgd_result["total_return"]:+.1f}%, Alpha={sgd_alpha:+.1f}%'
        )
        results["SGD Optimized"] = {
            "return": sgd_result["total_return"],
            "alpha": sgd_alpha,
            "max_dd": sgd_result["max_drawdown"],
            "win_rate": sgd_result["win_rate"],
            "method": "sgd",
            "weights": best_weights,
            "threshold": optimal_threshold,
        }

    default_result = run_backtest_with_weights(
        data_cache, symbols, test_days, 0.03, 0.08, 70, None
    )
    if default_result:
        default_alpha = default_result["total_return"] - spy_test_return
        print(
            f'  Default Weights (Test): Return={default_result["total_return"]:+.1f}%, Alpha={default_alpha:+.1f}%'
        )
        results["Default Weights (Test)"] = {
            "return": default_result["total_return"],
            "alpha": default_alpha,
            "max_dd": default_result["max_drawdown"],
    ...
    if OPTUNA_AVAILABLE:
        train_spy = spy_data[
            (spy_data.index >= train_days[0]) & (spy_data.index <= train_days[-1])
        ]
        train_spy_return = (
            train_spy.iloc[-1]["close"] / train_spy.iloc[0]["open"] - 1
        ) * 100
        ...
        def objective(trial):
            ...
            result = run_backtest_with_weights(
                data_cache, symbols, train_days, sl, tp, ms, weights
            )
            if result is None:
                return -100.0
            return (
                result["total_return"]
                - train_spy_return
                - max(0, -result["max_drawdown"] - 20) * 0.5
            )
        ...
        print(
            f'  Best: SL={bp["stop_loss"]*100:.1f}%, TP={bp["take_profit"]*100:.1f}%, Score={bp["min_score"]:.1f}'
        )

        total_w = bp["w_rsi"] + bp["w_macd"] + bp["w_bb"] + bp["w_trend"]
        bay_weights = {
            "rsi": bp["w_rsi"] / total_w,
            "macd": bp["w_macd"] / total_w,
            "bb": bp["w_bb"] / total_w,
            "trend": bp["w_trend"] / total_w,
        }

        bay_result = run_backtest_with_weights(
            data_cache,
            symbols,
            test_days,
            bp["stop_loss"],
        ...
        if bay_result:
            ...
            print(
                f'  Bayesian (Test): Return={bay_result["total_return"]:+.1f}%, Alpha={bay_alpha:+.1f}%'
            )
            results["Bayesian Optimized"] = {
                "return": bay_result["total_return"],
                "alpha": bay_alpha,
                "max_dd": bay_result["max_drawdown"],
                "win_rate": bay_result["win_rate"],
                "method": "bayesian",
                "params": bp,
            }
    ...
    regime_result = run_regime_adaptive_backtest(
        data_cache, symbols, trading_days, spy_data
    )
    if regime_result:
        regime_alpha = regime_result["total_return"] - spy_return
        print(
            f'  Regime-Adaptive: Return={regime_result["total_return"]:+.1f}%, Alpha={regime_alpha:+.1f}%'
        )
        print(f'  Regime Distribution: {regime_result["regime_counts"]}')
        results["Regime-Adaptive"] = {
            "return": regime_result["total_return"],
            "alpha": regime_alpha,
            "max_dd": regime_result["max_drawdown"],
            "win_rate": regime_result["win_rate"],
            "method": "regime_adaptive",
            "regime_counts": regime_result["regime_counts"],
        }
    ...
    if XGBOOST_AVAILABLE:
        ...
        X_ml, y_ml = ml_model.prepare_training_data(
            data_cache, symbols, train_days, 0.03, 0.08
        )
        ...
        if ml_model.model:
            ...
            for name, imp in sorted(
                zip(ml_model.feature_names, importance),
                key=lambda x: x[1],
                reverse=True,
            ):
                print(f"    {name}: {imp:.3f}")
            results["ML Model (XGBoost)"] = {
                "method": "ml",
                "feature_importance": dict(
                    zip(ml_model.feature_names, [float(x) for x in importance])
                ),
            }
    ...
    for name, r in sorted(
        results.items(), key=lambda x: x[1].get("alpha", -999), reverse=True
    ):
        if "return" in r:
            print(
                f'{name:<30} {r["return"]:>+9.1f}% {r["alpha"]:>+9.1f}% {r["max_dd"]:>9.1f}% {r["win_rate"]:>9.1f}%'
            )
