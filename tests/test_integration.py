...
def mock_alpaca_client():
    ...
    with patch("alpaca.trading.client.TradingClient") as mock:
        ...
        clock = MagicMock()
        clock.is_open = True
        clock.next_open = datetime.now(timezone.utc) + timedelta(hours=12)
        clock.next_close = datetime.now(timezone.utc) + timedelta(hours=4)
        clock.timestamp = datetime.now(timezone.utc)
...
def mock_data_provider():
    ...
    with patch("src.data_provider.alpaca_provider") as mock:
        # Mock latest trade
        mock.get_latest_trade.return_value = {"price": 150.00, "timestamp": datetime.now(timezone.utc).isoformat()}
...
class TestTradingLoopIntegration:
    ...
    def test_market_open_trading_cycle(
        ...
        with patch("src.strategy.strategy") as mock_strategy:
            mock_strategy.should_buy.return_value = (True, 75.0, "Strong technicals and sentiment")
...
class TestPerformanceTracking:
    ...
    def test_daily_pnl_calculation(self):
        ...
        trades = [
            {"symbol": "AAPL", "side": "sell", "qty": 10, "price": 155.00, "cost_basis": 150.00},
            {"symbol": "GOOGL", "side": "sell", "qty": 5, "price": 140.00, "cost_basis": 145.00},
        ]
...
class TestMarketRegimeIntegration:
    ...
    def test_bullish_regime_detection(self, mock_data_provider):
        ...
        bars_data = {
            "close": [100.0, 102.0, 105.0, 108.0, 112.0, 115.0, 118.0, 120.0, 122.0, 125.0],
        }
        mock_data_provider.get_bars.return_value = pd.DataFrame(bars_data)
    ...
    def test_bearish_regime_detection(self, mock_data_provider):
        ...
        bars_data = {
            "close": [125.0, 122.0, 118.0, 115.0, 110.0, 105.0, 100.0, 95.0, 92.0, 90.0],
        }
        mock_data_provider.get_bars.return_value = pd.DataFrame(bars_data)
